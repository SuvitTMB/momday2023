...
function loadData() {
  ...
  .get().then((snapshot)=> {
    snapshot.forEach(doc=> {
      ...
      xNewText += '<div style="color:#0056ff;font-size:13px;"><div><b>'+doc.data().EmpName+'</b></div>';
      xNewText += '<div><img src="'+doc.data().SendImg+'" class="send-img"></div>';
      xNewText += '<div style="max-width:400px; width:100%;margin-top:8px;font-size:13px;line-height: 1.2; color:#555;">'+doc.data().SendMemo+'</div>';
      xNewText += '<div style="margin-top:4px;color:#f68b1f;font-weight: 600;">View : '+doc.data().ClickView+' | Like : '+doc.data().ClickLike+' | ข้อความ : '+doc.data().ClickMemo+' </div></div>';
